...
vector<double> getFrenet(double x, double y, double theta, vector<double> maps_x, vector<double> maps_y, vector<double> maps_s)
{
	...
	// calculate s value
	double frenet_s = maps_s[0]; //ATTENTION
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
					...
					// This calculation helps to create the ego car object in the next section
				  ...
				  for (int i = -NUM_WAYPOINTS_BEHIND; i < NUM_WAYPOINTS_AHEAD; i++) {
					...
					double duration = PLAN_AHEAD_DURATION;// - subpath_size * PATH_DT;
					...
					// If any other car is immediately to left or right, set a 
					...
					my_car.car_just_ahead = car_just_ahead;
					my_car.car_to_left = car_to_left;
					my_car.car_to_right = car_to_right;

					// ******************************* DETERMINE BEST TRAJECTORY ***********************************

					vector<vector<double>> best_target = my_car.get_best_frenet_target_from_trajectory(predictions, duration);
					...
					// last two points of coarse trajectory, use target_d and target_s,target_s + 30

					double target_s1 = best_target[0][0];
